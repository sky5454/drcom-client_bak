...
MODULE_DESCRIPTION("TCP Track" TCPTRACK_VERSION);
...
// extern int dump_ip_packet(const char *msg, const struct sk_buff *skb, int offset);
...
#define TODO_SEND_AUTH	0x01
#define TODO_HANDSHAKE	0x02
...
#if 0
static void DUMP_TUPLE(struct tcp_tuple *t)
{
	DEBUGP("Tuple dump:\n");
	DEBUGP("  src_ip=%u.%u.%u.%u\n",NIPQUAD(t->src_ip));
	DEBUGP("  dst_ip=%u.%u.%u.%u\n",NIPQUAD(t->dst_ip));
	DEBUGP("  src_port=%d\n",ntohs(t->src_port));
	DEBUGP("  src_port=%d\n",ntohs(t->dst_port));
	DEBUGP("  dir=%s\n", t->dir?"REPLY":"ORIG");
}
...
/* return what to do */
static unsigned int tcp_packet(struct tcp_conn *conn, int dir, struct sk_buff *skb)
{
	...
	if (tcph == NULL)
        ...
        if (skb->len < (iph->ihl + tcph->doff) * 4) {
	...
	if (oldtcpstate == TCP_STATE_NONE && dir == CONN_DIR_ORIG && tcph->syn && !tcph->ack) {
        ...
        if (oldtcpstate == TCP_STATE_SYN_RECV && dir == CONN_DIR_ORIG
        ...
        if (oldtcpstate == TCP_STATE_ESTABLISHED && dir == CONN_DIR_ORIG
                    && tcph->ack && !tcph->syn && ntohl(tcph->ack_seq) == conn->seq[!dir].syn_seq+1
	...
	DEBUGP("old_state=%s, new_state=%s\n", tcp_state_names[oldtcpstate], tcp_state_names[newstate]);
		...
		unsigned int sackoff, unsigned int sackend, struct tcp_seq *seq)
{
        while (sackoff < sackend) {
                ...
                DEBUGP("sack_adjust: start_seq: %d->%d, end_seq: %d->%d\n",
                         ntohl(sack->start_seq), new_start_seq, ntohl(sack->end_seq), new_end_seq);
...
static int tcp_adjust_seq(struct tcp_conn *conn, int dir, struct sk_buff *skb)
{
        struct tcphdr *tcph;
...
static void conn_init_tuples(struct tcp_conn *conn, struct tcp_tuple *tuple)
{
	struct tcp_tuple *t;
...
static int __add_new_hash(struct tcp_tuplehash *hash)
{
	...
	list_for_each(pos, head){
		struct tcp_tuplehash *e = list_entry(pos, struct tcp_tuplehash, list);
		if (tuple_equal(&e->tuple, &hash->tuple))
			break;
	}

	if (pos != head) {
		DEBUGP(KERN_INFO "tuple exists\n");
		return 0;
	}
...
static struct tcp_tuplehash *conn_find_get(struct tcp_tuple *tuple)
{
	...
	list_for_each(pos, head){
		...
		if (tuple_equal(&tuplehash->tuple, tuple)) {
			conn_get(tuplehash_to_conn(tuplehash));
...
static inline struct tcp_conn *conn_alloc(struct tcp_tuple *tuple)
{
...
static struct tcp_conn *resolve_tcp_conn(struct sk_buff *skb, struct tcp_tuple *tuple, struct tcp_tuplehash **phash)
{
	struct tcp_conn *conn;
	struct tcp_tuplehash *hash;

	hash = conn_find_get(tuple);
	if (hash == NULL) {
		if (!is_syn_pkt(skb)) /* syn & !ack */
			return NULL;

		conn = conn_alloc(tuple);
		if (conn == NULL)
			return NULL;

		conn_get(conn);/* for us */
		conn_get(conn);/* for hash list */

		write_lock_bh(&hash_lock);
		if (!__add_new_hash(&conn->tuplehash[CONN_DIR_ORIG])
			|| !__add_new_hash(&conn->tuplehash[CONN_DIR_REPLY]))
		{
			write_unlock_bh(&hash_lock);
			conn_put(conn); /* for hash list */
			conn_put(conn); /* for us */
			return NULL;
		}
...
/* returns:
 * 	 -1 	when error, and oskb should be NF_ACCEPT.
 * 	  0 	oskb is a pure ack of SYN, oskb has been NF_STOLEN.
 *	  1	oskb contains data, it should adjust seq.
 */
static int build_send_auth_skb(struct tcp_conn *conn, struct sk_buff *oskb, int (*okfn)(struct sk_buff *))
{
...
/*
	...
	diff = skb->len - (iph->ihl << 2) - (tcph->doff << 2);
	DEBUGP("Handshake ACK of SYN-ACK carries data %d byte(s)\n", diff);
	if (diff > 0) {
		struct sk_buff *skb2;
		...
		skb2 = skb_copy_expand(skb, skb_headroom(skb), CONN_AUTH_DATA_LEN, GFP_ATOMIC);
		if (skb2 == NULL)
			return -1;

		DEBUGP("pure ACK built, Sending it\n");
		okfn(skb);
		...
		DEBUGP("Sending original pure ACK\n");
		okfn(oskb);
...
*/
	...
	hdrlen = ip_hdrlen(skb) + (tcph->doff << 2);
	memcpy(skb->data - CONN_AUTH_DATA_LEN, skb->data, hdrlen);
	memcpy(skb->data - CONN_AUTH_DATA_LEN + hdrlen, conn_auth_data, CONN_AUTH_DATA_LEN);
	...
	/* tcp stuff */
/*
	tcph->seq = htonl(conn->seq[CONN_DIR_ORIG].syn_seq+1);
	tcph->psh = 1;
*/
...
static int is_our_packet(struct sk_buff *skb)
{
	struct iphdr *iph = ip_hdr(skb);

	return (iph->protocol == IPPROTO_TCP || iph->protocol == IPPROTO_TCP);
}
				...
				int (*okfn)(struct sk_buff *))
{
	...
	conn = resolve_tcp_conn(skb, &tuple, &hash);
	if (conn == NULL)
		goto out_unlock;

	todo = tcp_packet(conn, hash->tuple.dir, skb);

	if (!(todo & TODO_HANDSHAKE))
		(void)tcp_adjust_seq(conn, hash->tuple.dir, skb);
				...
				int (*okfn)(struct sk_buff *))
{
	...
	if (todo & TODO_SEND_AUTH) {
		...
		r = build_send_auth_skb(conn, skb, okfn);

		if (r == 0) {
			ret = NF_STOLEN;
			goto out_put;
		} else if (r == -1) { /* error, bypass seq adjust */
			ret = NF_ACCEPT;
			goto out_put;
		}

		/* r == 1, adjust seq of first data pkt */
	}
...
do_nothing:
	...
	if (hash_todo == 1) {
		...
		conn_hooks_init();
		printk(KERN_INFO "Drcom-TcpTrack: Start Auth\n");
	} else if (hash_todo == 2) {
		del_keepalive_timer();
		conn_hooks_cleanup();
		conn_hash_cleanup();
		printk(KERN_INFO "Drcom-TcpTrack: Stop Auth\n");
	}
...
static int __init init(void)
{
	...
	printk(KERN_INFO "Drcom-TcpTrack-" TCPTRACK_VERSION " module loaded\n");
...
static void __exit fini(void)
{
	...
	printk(KERN_INFO "Drcom-TcpTrack-" TCPTRACK_VERSION " module unloaded\n");
}
